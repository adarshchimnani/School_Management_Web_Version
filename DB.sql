...
BEGIN
    SELECT subject_name,total_marks,obtained_marks,grade,concat(first_name,' ',last_name)AS s_name,p.name AS p_name
            from results r INNER JOIN students USING(s_id) INNER JOIN parent p USING(p_id)
                WHERE r.s_id=id AND r.class_id=c_id AND r.term_year=year_ AND r.session_=sson;
END$$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE emp_d(IN e_id int(15))
BEGIN
        Select * from employees where id=e_id;
END$$
DELIMITER ;

DELIMITER $$
CREATE PROCEDURE stu_d(IN id int(15))
BEGIN
        Select s_id, first_name, last_name, gender, birth_date, last_school, class_Addm, p_id, name, cnic, contact, email, address
        from students inner join parent using (p_id) where s_id=id;
END$$
DELIMITER ;


DELIMITER $$
CREATE PROCEDURE sub_r(IN year varchar(15))
BEGIN
        Select subject, total_marks, concat(first_name,' ',last_name) as name, class_id, term_year
        from subjects inner join employees on t_id=id where term_year=year;
END$$
DELIMITER ;


DELIMITER $$
CREATE PROCEDURE sub_insert(IN sub varchar(15), IN teacher int(3), IN marks int (3), IN class varchar (15), IN year varchar (15))
BEGIN
       INSERT INTO subjects VALUES(sub, teacher, marks, class, year);
END$$
DELIMITER ;
